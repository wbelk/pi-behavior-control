// Post-edit review reminder text.
// Spec: section 7 hook 6 of tasks/plan-pi-behavior-control.md.

const REVIEW_BRIEF = `Review this file as a senior engineer and architect before submission.
Review for style, elegance, and efficiency. Is every line of code consistent
with the intended design and architecture? Trace every caller, upstream, and
downstream execution path in this file -- does this code match the intent?
Does the code have any conditional logic that supports soft-failing instead
of fixing the root cause of any potential issues? Are any arguments
conditional that should be required? Broken logic should break the app and
the tests. Always try to achieve DRY -- do not repeat yourself. Set reused
references to vars, always try to create reusable and shareable
modules/methods/functions. Write tests that match code intent, included all
edge cases and functional scenarios — tests should not just test the code as
written. Fix any issues, soft-failure scenarios, future-proofing without
explicit user direction, and errors with the code before submitting.
...
- If your prior response claimed completion and test suite exists in project: confirm test suite ran with 0 failing this turn. If not, retract the claim.
